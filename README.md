...
For a link anyone can open from any device or network — which is what a
demonstration usually needs — deploy it. The built web application is served by
the API process, so this is a single web service: one URL, one port, HTTPS from
the platform.

### Render

The repository carries a blueprint. On [render.com](https://render.com):
**New → Blueprint → select this repository**. It reads `render.yaml`, and asks
only for `SEED_PASSWORD` — the password the seeded operator accounts will get.
`JWT_SECRET` is generated by Render and never stored in git.

The free plan sleeps after inactivity and its filesystem is ephemeral, so the
database is rebuilt from the seed on each cold start. Accounts come back;
...
Raspberry Pi on the demonstration bench all work the same way:

```bash
docker build -t smart-er .
docker run -p 4000:4000 \
  -e NODE_ENV=production \
  -e JWT_SECRET="$(openssl rand -hex 32)" \
  -e SEED_PASSWORD="pick-your-own-long-one" \
  -v smart-er-data:/app/data \
  smart-er
```

The volume is what makes the database survive a restart.

Whatever the host, it must run **Node 22.5 or newer** and set
`NODE_ENV=production` — which is what makes the server refuse to start without
a real `JWT_SECRET` rather than falling back to a constant.
